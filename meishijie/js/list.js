...
//	判断是否是菜单详情页
	for (var x=0;x<loc.length;x++) {
		for (var y=0;y<loc[x].split(/\=/).length;y++) {
			var reg = /menu/
			var not = reg.test(location.search.replace(/\?/,""))
...
//	}
	...
	//	大标题鼠标点击
		$(".list_tit").off("mousedown").on("mousedown","li",function(){
			$(".list_main_tit h3:first-child").html("印象中的那些妈妈的味道")
	...
	//		改变主要内容的标题
			$(".list_main_tit h3").eq($(this).index()).addClass("dis").siblings().removeClass("dis")
	//		改变主要内容
			cid = $(this).attr("cid")
			name ="data=cid="+cid
			cidajax();
